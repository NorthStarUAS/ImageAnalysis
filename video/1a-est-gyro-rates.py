...
parser.add_argument('--write', action='store_true', help='write out the smoothed video')
...
output_video = filename + "_smooth" + ext
...
def edge1(gray):
    ...
    if accum_e is None:
        accum_e = np.float64(edges.copy())
    else:
        accum_e += edges
    counter_e += 1
    print("max edge:", np.max(accum_e) / counter_e)
    edges = np.uint8(accum_e / counter_e)
    cv2.imshow("edges:", np.uint8(accum_e / counter_e))
    max_val = np.max(edges)
    thresh_val = int(round((max_val * 0.25)))
    ret,thresh1 = cv2.threshold(edges,thresh_val,255,cv2.THRESH_BINARY)
    cv2.imshow("edge mask:", thresh1)
    #ret2, thresh = cv2.threshold(edges, 0, 255, cv2.THRESH_BINARY+cv2.THRESH_OTSU)
    #cv2.imshow("edge otsu:", thresh)
...
for frame in reader.nextFrame():
    ...
    if args.equalize:
        # just splits in the HSV and equalizes the value only,
        # recombines, returns, nothing special.
    ...
    if abs(rot) > 10 or sqrt(tx*tx+ty*ty) > 30:

...
model = classifier1.Classifier()
model.init_model(basename="ob4")
model.compute_lbp(rgb, radius=3)
model.compute_redness(rgb)
model.compute_grid(grid_size=256)
cv2.imshow('model', cv2.resize(model.red_index.astype('uint8'), (int(w*args.scale), int(h*args.scale))))
